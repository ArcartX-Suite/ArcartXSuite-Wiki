---
title: index | ArcartX-Suite Minecraft插件架构文档
...
- [CrossServer — 统一跨服 SDK](cross-server)：全模块共用 Redis/Proxy 双后端与 JSON 信封
...
- [Config Autofix — 配置智能诊断](config-autofix)
- [UI Packet 数据全景](/ui-packet-data)：各模块 UI 变量与回包动作自动索引

## 开发者 API

提供独立的 `ArcartX-Suite-api` 模块作为第三方开发的稳定接口层，详见 [API 参考](/api/)。
...
ArcartX-Suite 用 **HikariCP + SQLite/MySQL 共存**：

- 默认 `mode: sqlite`，文件位于 `plugins/ArcartX-Suite/<module>.db`
- 改 `mode: mysql` 后填连接信息即可切换
- 所有模块用各自独立的连接池

涉及持久化的模块：`title` / `mail` / `chat` / `onlinerewards` / `loginview` / `map` / `warehouse` / `market` / `battlepass` / `fishing` / `lottery` / `afkreward` / `menu` / `regions`
