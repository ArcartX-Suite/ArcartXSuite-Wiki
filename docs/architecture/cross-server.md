...
| 原则 | 说明 |
| --- | --- |
| **子服侧总线** | 跨服逻辑在子服 `axs-core` 的 `CrossServerService` 中实现，模块只注册通道与业务 payload |
...
| **无旧版兼容** | 已移除各模块 `transport.redis` / `transport.proxy` 等独立配置 |
...
| `proxy.messenger-channel` | 默认 `AXS_CROSS`，与旧版各模块独立频道不同 |
