...
::: tip 开发教程
本文是 **API 速查**。原理、内置能力关系图、提供方/使用方完整示例见 **[Capability 详解](/guide/developer/capability-guide)**。
:::
...
public interface SignalDispatchable {
    void dispatchSignal(@NotNull String signal, @NotNull Player subject,
                        @Nullable Map<String, String> variables);
}
```

**使用场景：** OnlineRewards 等模块在签到成功等时机触发 EventPacket 规则引擎。

### MenuOpenable

由 Menu 模块注册，供 Essentials 等模块打开配置驱动的 ArcartX 菜单。
...
由 AfkReward 模块注册，供 Essentials 查询挂机状态或触发原地挂机。

```java
public interface AfkRewardDispatchable {
    boolean isAfk(@NotNull UUID playerUuid);
    @Nullable String getAreaName(@NotNull UUID playerUuid);
    int getAfkSeconds(@NotNull UUID playerUuid);
    @Nullable String getAfkMode(@NotNull UUID playerUuid);
    boolean startManualAfk(@NotNull Player player, @NotNull String areaName);
}
```

**使用场景：** Essentials 的 AFK 踢人与 AfkReward 挂机状态互斥判断。
