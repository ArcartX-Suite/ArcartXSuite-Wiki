...
ArcartX-Suite 提供 **`ArcartX-Suite-api`** 模块作为第三方开发的稳定接口层。所有公开 API 均位于 `xuanmo.arcartxsuite.api` 包下，第三方模块只需 `compileOnly` 依赖 `ArcartX-Suite-api` 即可开发自定义模块。

::: tip 动手教程
若你是第一次开发模块，建议先读 [开发者指南](/guide/developer/)，并克隆 [ArcartXSuite-Core](https://github.com/ArcartX-Suite/ArcartXSuite-Core) 获取 `axs-api` 源码，再回来查阅本 API 参考。
:::
...
| `@ApiStability.Internal` | 内部 API，仅供 ArcartX-Suite 内部模块使用，第三方不应依赖 |
...
| [`CrossServerAPI`](./module-context#跨服-api-crossserverapi) | `api.crossserver` | 统一跨服 SDK |
| [`CrossServerChannel`](./module-context#跨服-api-crossserverapi) | `api.crossserver` | 模块跨服发布/订阅句柄 |
