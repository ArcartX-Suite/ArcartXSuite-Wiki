...
AXS 1.2.0-beta 支持模块 Jar 独立部署。不放入模块 Jar 时，全部功能由宿主内置加载。
...
plugins/
  ...
  ArcartX-Suite/
    config.yml
    modules/                ← 按需放入模块 Jar
      ArcartXSuite-RGB-1.2.0-beta.jar
      ArcartXSuite-Tab-1.2.0-beta.jar
      ArcartXSuite-Pickup-1.2.0-beta.jar
      ...
```
...
- **纯 LittleSkin**：直接在启动命令中加入 `-javaagent`：
  ...
  ```
- **Mixed Auth（LittleSkin + 微软正版）**：后端子服使用 AXS 生成的 `start-mixed-auth` 脚本启动（先启动本地混合代理，再启动服务器）。详见 [LoginView 模块的 Mixed Auth 配置](../modules/loginview.md#mixed-auth-mode混合认证)。
