...
| SDK | `axs-api.jar` — [ArcartXSuite-Core Releases](https://github.com/xuanmomo233/ArcartXSuite-Core/releases)（推荐）；或从完整 `ArcartXSuite` 工程 `./gradlew :axs-api:jar` 构建 |
...
::: tip 加载顺序
宿主按 `depends` 做拓扑排序。若模块 A `depends: [title]`，则 Title 一定先于 A 启用——这对 Capability 消费方很重要。详见 [Capability 详解](./capability-guide#加载顺序与-softdepends)。
:::
...
3. 消费方：`context.getCapability(接口.class)`，务必判空
