...
::: warning 当前获取方式
[ArcartXSuite-Core](https://github.com/xuanmomo233/ArcartXSuite-Core) 仓库**目前只开源了 `axs-api` 源码**，尚未提供预编译 `axs-api.jar` 或 GitHub Releases。第三方开发者暂时无法「下载即用」。

**推荐补齐（仓库维护方）：** 在 Core 仓库增加 Gradle 构建 + GitHub Actions，每个版本发布 `axs-api-<version>.jar` 到 Releases。

**临时方案（开发者）：** 若你持有完整 `ArcartXSuite` 工程，可在根目录执行 `./gradlew :axs-api:jar`，产物位于 `axs-api/build/libs/axs-api.jar`，复制到模块工程的 `libs/` 即可。
:::
...
1. 获取 `axs-api.jar`（见上方说明）：优先从 [ArcartXSuite-Core Releases](https://github.com/xuanmomo233/ArcartXSuite-Core/releases) 下载；若尚未发布，则从完整 `ArcartXSuite` 工程执行 `./gradlew :axs-api:jar` 自行构建。
