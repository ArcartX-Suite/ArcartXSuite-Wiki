...
::: tip 开源 SDK
完整 API 源码与示例见 GitHub：[ArcartXSuite-OpenSource](https://github.com/xuanmomo233/ArcartXSuite-OpenSource)。开发时只需 `compileOnly` 依赖 `axs-api` JAR，**禁止**直接引用宿主实现类（`xuanmo.arcartxsuite.bridge.*` 等）。
:::
...
1. 从 [OpenSource Releases](https://github.com/xuanmomo233/ArcartXSuite-OpenSource/releases) 下载 `axs-api` JAR。
