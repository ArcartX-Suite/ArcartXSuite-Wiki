...
::: tip 开源 SDK
完整 API 源码与 `MODULAR-README.md` 见 GitHub：[ArcartXSuite-Core](https://github.com/xuanmomo233/ArcartXSuite-Core)。开发时只需 `compileOnly` 依赖 `axs-api` JAR（从仓库 `axs-api/` 构建或自行编译），**禁止**直接引用宿主实现类（`xuanmo.arcartxsuite.bridge.*` 等）。
:::
...
1. 克隆或下载 [ArcartXSuite-Core](https://github.com/xuanmomo233/ArcartXSuite-Core)，在 `axs-api/` 构建得到 `axs-api` JAR（或按仓库 README 说明获取）。
