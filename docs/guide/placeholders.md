...
> **前置条件**：服务器需安装 [PlaceholderAPI](https://www.spigotmc.org/resources/placeholderapi.6245/) 插件，ArcartX-Suite 会在启动时自动注册占位符。
...
部分 ArcartX-Suite 模块会**读取其他插件的 PAPI 占位符**作为数据来源，这不需要额外配置——只需确保对应的占位符插件已安装。
