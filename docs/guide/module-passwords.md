---
title: 模块启用与授权 | ArcartXSuite Minecraft插件文档
description: 模块启用与授权 - ArcartXSuite Minecraft 服务器插件文档。 ArcartXSuite 我的世界服务器插件套件。
---
...
当前付费模块：`warehouse`、`map`、`mail`、`title`、`questgps`、`conversation`、`market`、`qqbot`、`battlepass`、`fishing`、`lottery`。

当前福利模块：`tab`、`entitytracker`。
