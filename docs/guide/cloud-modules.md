...
2. **已获得模块授权** — 免费模块自动发放；付费模块需购买；福利模块按累计消费领取（见下文）。
...
以下模块为**福利模块**，按累计消费在云端领取；每次领取会发放固定数量的**授权份额**（每份可装备到一台服务器）：
...
在「装备模块」页面可查看各模块 **已用 / 共 N 份** 席位。多服网络需为每台子服分别装备，并确保 `config.yml` 中对应模块 `enabled: true`。
