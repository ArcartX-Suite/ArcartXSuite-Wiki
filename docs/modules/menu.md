...
- 打开条件：权限 + PlaceholderAPI / **Aria 脚本** 表达式（见 [条件系统](/guide/conditions)）
- **命令绑定**：精确命令 + 正则命令拦截
- **物品绑定**：手持/副手物品右键打开菜单
...
Menu 的条件系统与 Prop / EventPacket / Mail **共用同一引擎**，完整语法见 **[条件系统（PlaceholderAPI + Aria）](/guide/conditions)**。  
...
| 位置 | 可用 Aria 相关键 |
| --- | --- |
| 按钮可见 | `requirements`、`view-conditions`、`conditions`、`aria-conditions` |
| 按钮使用 | `condition`、`use-conditions`、`click-conditions`、`aria-condition` |
| 菜单打开 | `open-requirements`、`aria-conditions` |

::: warning
Aria 脚本内不会自动展开 `%placeholder%`。混用 PAPI 行 + Aria 脚本时仍为 AND。
:::
...
| 按钮灰色点不了 | **使用条件** `condition` 未通过；查看 `deny-message`；Aria 未部署时 Aria 条件恒失败 |
| 按钮不显示 | **可见条件** `requirements` 未通过 |
| PAPI 条件异常 | 确认 PAPI 与 Expansion；见 [条件系统](/guide/conditions#故障排查) |
| Aria 条件全失败 | 确认 `BlinkAriaHost` 已加载；见 [条件系统 · Aria](/guide/conditions#二aria-脚本条件) |
