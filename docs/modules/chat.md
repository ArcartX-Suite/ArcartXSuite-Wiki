...
::: tip 福利模块
本模块为福利模块（**任意消费**即可在 [云端平台](https://cloud.021209.xyz) 领取 **5 个** Chat 授权份额）。领取后于「装备模块」勾选到服务器即可，无需 `license.yml`。每份授权可装备到一台服务器。
:::
...
- **跨服转发**：通过宿主 [CrossServer SDK](/architecture/cross-server)（Redis `ArcartX-Suite:CROSS` / Proxy `AXS_CROSS`），多服消息互通
...
在 `config.yml` 启用模块，并确认云端已装备 Chat 授权：
...
详细示例见 [跨服功能配置指南](/guide/cross-server-setup)。
