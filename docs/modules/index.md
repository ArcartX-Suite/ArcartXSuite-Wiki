...
- **Ed25519 签名验证**：服务器与云端通信使用 Ed25519 密钥对签名，支持模块签名验证防止篡改
...
| 模块 | 拆分的配置 | 新目录结构 |
| --- | --- | --- |
| Announcer | `entries:`、`subtitle.groups:` | `data/announcer/announcer/*.yml`、`data/announcer/subtitle/groups/*.yml` |
...
| EventPacket | ArcartX | PlaceholderAPI、MythicMobs/MythicBukkit、其他 ArcartX-Suite 模块 | 只在使用对应触发器或动作时需要 |
...
| QuestGPS | ArcartX、Chemdah | Map、EventPacket | Chemdah 为 SSOT；QuestGPS overlay 白名单 + UI/导航 |
