---
title: 模块总览 - ArcartXSuite 26个Minecraft服务器插件 | 我的世界插件套件
description: ArcartXSuite 提供 26 个 Minecraft 服务器功能模块，涵盖聊天、战斗、播报、经济交易、全球市场、抽奖开箱、钓鱼、称号、战令、仓库等核心玩法，是 Minecraft ArcartX 服务器开发者的首选插件系列。
---

# 模块

AXS 共 **26 个功能模块**，涵盖聊天、战斗、播报、追踪、登录、基础工具、区域保护、通用菜单、经济交易、QQ群联动、钓鱼、抽奖等核心玩法。其中 **13 个免费模块** 开箱即用，**11 个付费模块 与 2 个福利模块** 需要单独授权。
...
| 模块 | 授权 | 必需依赖 | 按功能选装 | 说明 |
| --- | --- | --- | --- | --- |
| **Essentials** | 免费 | ArcartX | PlaceholderAPI、Tab、Chat | 基础玩家工具集合 |
| **Regions** | 免费 | ArcartX | — | 区域保护 + 世界规则 |
| **Menu** | 免费 | ArcartX | PlaceholderAPI、BlinkAriaHost（Aria 条件）、MythicMobs/NeigeItems/MMOItems | 配置驱动菜单；[条件系统](/guide/conditions) |
| **AfkReward** | 免费 | ArcartX | PlaceholderAPI | PAPI 用于奖励命令解析和对外输出 |
| Announcer | 免费 | ArcartX | PlaceholderAPI、EventPacket | PAPI 只影响文本变量解析 |
| Chat | 免费 | ArcartX | PlaceholderAPI、MySQL | MySQL 用于跨服共享禁言/状态；跨服走宿主 cross-server |
...
| OnlineRewards | 免费 | ArcartX | PlaceholderAPI、Mail、Vault、MySQL | 邮件奖励；跨服需 MySQL + 宿主 cross-server |
...
| Mail | 付费 | ArcartX、有效模块授权 | PlaceholderAPI、Vault、PlayerPoints、MySQL、物品库插件 | 跨服邮件建议 MySQL + 宿主 cross-server |
...
| **Market** | 付费 | ArcartX、MySQL、有效模块授权 | Redis（缓存）、Vault、PlayerPoints、PlaceholderAPI、MythicMobs、NeigeItems | Redis 可选列表缓存；跨服走 cross-server |
| **Lottery** | 付费 | ArcartX、有效模块授权 | Mail、MythicMobs、NeigeItems、PlaceholderAPI | 背包满时自动发邮件；物品库用于奖品来源；PAPI 用于输出统计 |
| **QQBot** | 付费 | ArcartX、OneBot 11 实现端、有效模块授权 | MySQL、PlaceholderAPI、Chat | OneBot 实现端需自行部署（Lagrange/NapCat/LLOneBot） |
| EntityTracker | 福利 | ArcartX | MythicMobs/MythicBukkit、PlaceholderAPI、NeigeItems、MythicLib/MMOItems、宿主 CrossServer、Vault | Boss 追踪需要 Mythic；普通攻击目标 HUD 可独立使用；跨服排行需宿主 cross-server；Vault 用于金钱奖励 |
| Tab | 福利 | ArcartX、PlaceholderAPI、有效模块授权 | Title | 跨服快照走宿主 cross-server（建议 Redis） |
...
<div class="module-grid">
  <a href="entitytracker" class="module-card">
    ...
    <span class="card-badge badge-premium">💎 消费额度满¥180</span>
  </a>
  <a href="tab" class="module-card">
    ...
    <span class="card-badge badge-premium">💎 消费额度满¥320 或 找我购买ArcartX高级会员</span>
  </a>
</div>