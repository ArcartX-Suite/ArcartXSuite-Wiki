...
**通用触发器 + 动作链**模块。当指定事件发生时，按顺序执行一组动作（发 UI 包、播字幕、执行命令、派发邮件、授予称号等）。支持 13 种触发器和 13 种动作类型，覆盖服务端事件监听、客户端回包驱动、Chemdah 任务联动和脚本条件触发四大场景。每条规则还支持与 `prop` 模块一致的 `conditions` 条件判断，可先校验 PlaceholderAPI 值再决定是否执行动作。此外还内置了**实体清理（ClearLag）**和**定时命令**两项服务端实用功能。
...
| 可选 | Aria | `script-aria` 触发器 | Aria 脚本触发器不生效，JS 触发器不受影响 |
...
- `script-js` 使用 Java Nashorn 引擎，脚本中可通过 `player` 变量访问当前玩家对象。
- `script-aria` 需要 Aria 插件，使用 Aria 脚本语言，同样绑定 `player` 变量。
