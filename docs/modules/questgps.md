...
- **A* 路径寻路**：从玩家到目标沿地面生成多个导航标记实体，智能绕开障碍物、液体和危险方块，支持 ±1 格台阶处理，玩家移动时动态更新路径
...
navigation:
  ...
  marker:
    ...
    path-max-iterations: 2000            # A* 最大迭代次数
...
| `path-max-iterations` | int | `2000` | A* 寻路最大迭代次数（越大路径越精确但越耗时） |
...
玩家在菜单中点击"追踪任务"或"追踪目标"
  ...
  │
  ├─② A* 路径寻路 + 3D 模型标记
  │   异步 A* 计算路径 → 沿路径采样点生成多个 ArmorStand 私有实体
...
1. **A* 寻路** — 异步计算从玩家到目标的地面路径，智能绕开固体障碍物、液体、危险方块（仙人掌、岩浆块、营火等），支持 ±1 格台阶
