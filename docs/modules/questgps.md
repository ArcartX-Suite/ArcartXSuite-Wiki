...
1. **overlay ID** — 根键改为 Chemdah 裸 ID（如 `example_mainline_quest`），不再使用 `main/example_quest`
2. **分类** — `category.source` 二选一：`chemdah` 仅 `meta.type`，`overlay` 仅 overlay `category`（已移除 id-prefix-rules）
3. **UI 发包** — `questRows` / `taskRows` / `rewardRows` 已改为 `quests` / `tasks` / `rewards` Map；分类 Tab 动态渲染
4. **配置新增** — `presentation.source`、`category.source`、`discovery.mode`、`navigation.mode`、`database.*`（已废弃 `presentation.*-source` 与 `merge`）
