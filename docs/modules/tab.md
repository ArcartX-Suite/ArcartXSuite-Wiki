...
> 无法识别的占位符会**原样保留**在文本中。Tab 模块不再调用外部 PlaceholderAPI，因此 Vault、EssentialsX 等第三方扩展的占位符（如 `%vault_primary_group%`）将不会被解析。
...
- **pvp-highlight**：玩家造成或受到伤害后，`window-ms` 内 `isPvpActive()` 返回 `true`。此状态用于 `/axstab debug` 调试输出，不再对外暴露 PAPI 占位符。
