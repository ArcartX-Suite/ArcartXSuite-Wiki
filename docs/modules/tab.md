...
::: tip 福利模块
本模块为福利模块（**累计消费满 ¥100** 可在 [云端平台](https://cloud.021209.xyz) 领取 **5 个** Tab 授权份额）。从 **1.2.0-beta** 起，领取后于「装备模块」勾选即可，无需 `license.yml`。每份授权可装备到一台服务器。
:::
...
- **过滤器**：`filters.include` / `filters.exclude` 按内置变量表达式或权限节点过滤；`hide-vanished: true` 一键隐藏隐身玩家
...
Tab 模块内置占位符解析器，**无需安装 PlaceholderAPI** 即可使用以下变量。这些变量可在 `pack`、`sort-keys`、`filters`、`grouping`、`aggregate.line-pack` 中直接使用。
...
> 无法识别的占位符会**原样保留**在文本中。Tab 模块内置解析器只处理上述变量与 `axstab_*` 占位符，不调用外部 PlaceholderAPI，因此 Vault、EssentialsX 等第三方扩展的占位符（如 `%vault_primary_group%`）将不会被解析。
...
在 `config.yml` 启用模块，并确认云端已装备 Tab 授权：
...
跨服连接在宿主 `config.yml` → `cross-server`；模块 `ArcartXTab.yml` 的 `cross-server.enabled` 为总开关。详见 [跨服通信架构](/architecture/cross-server)。
...
| `key` | papi | 内置变量表达式，如 `%player_health%` |
...
- **隐身集成**：`hide-vanished: true` 检测 Bukkit `player.metadata("vanished")`，命中即隐藏（绝大部分隐身插件都会写入此 metadata）。
...
`{group}` 在 `header-pack`（无论 string / list / map / key 名）中被替换为当前组键。header 内的内置变量按该组第一个玩家上下文渲染。
...
- **每行控件**：`head`（`PlayerSkin:` 头像）+ `name_text`（玩家名）+ `health_text`（仅详细模式 visible）。
...
Tab 模块内置 PVP 检测窗口，通过 `TabPvpListener` 监听玩家攻击/受击事件，并通过 `axstab` PAPI 扩展对外暴露视觉风格占位符。

| 占位符 | 说明 |
| --- | --- |
| `%axstab_pvp%` | 是否处于 PVP 高亮窗口（`true` / `false`） |
| `%axstab_pvp_color%` | `settings.style.pvp-highlight.color`，未启用或不在窗口时返回空串 |
| `%axstab_vanished%` | 是否处于隐身状态（检测 `player.metadata("vanished")`） |
| `%axstab_vanish_color%` | `settings.style.vanish-grey.color`，未启用或未隐身时返回空串 |
| `%axstab_ping%` | 当前玩家延迟（ms） |
| `%axstab_ping_icon%` | 按 `settings.style.ping-icon.tiers` 档位匹配的输出图标 |
...
开启后服务端**完整跑一遍渲染流程**（filters / sortKeys / pinned / grouping / pagination / 内置变量解析），但**不调用 `bridge.sendPacket(...)`**，客户端 Tab 保持上一次状态。配合 `settings.debug: true` 会逐 viewer 输出 `ArcartXTab[dry-run] skip send def=... viewer=...` 日志，用于：
