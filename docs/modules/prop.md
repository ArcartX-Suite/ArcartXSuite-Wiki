...
| `mythiclib.source-prefix` | string | `AXS_PROP` | MythicLib modifier 前缀，最终生成 `<前缀>_<道具ID>_<属性>`；是否生效由本体 `config.yml` → `attribute-bridges.mythiclib` 统一管理 |
...
conditions:
  ...
  # JavaScript 脚本（需要 classpath 提供 ScriptEngine）
...
- **Aria 条件**：由 ArcartX 内置，随 `depend: [ArcartX]` 提供，始终可用。Aria 路径会在求值前展开脚本文本中的 `%...%` PAPI 占位符和 `{player}`；`player` 是 AXS 的 `AriaPlayer` 门面，`player.getWorld()` 直接返回世界名字符串，门面未覆盖的原生 API 使用 `player.bukkit()`。
- **JS 条件**：需要 classpath 提供 JavaScript `ScriptEngine`；Java 15+ 默认无 Nashorn，可使用 GraalJS 或 standalone Nashorn。JS 路径不会做脚本级 `%...%` / `{player}` 预展开，取 PAPI 请使用 `player.papi()` 或 `player.papiNumber()`。JS 侧的 `player` 同样是 `AriaPlayer` 门面。

详见 [条件系统](/guide/conditions)。

Prop 的 `conditions` 使用共享的 PAPI / Aria / JS 条件系统；`effects` 则是 Prop 自己的 `type|payload` 效果 DSL，例如 `cmd`、`msg`、`food`、`health`、`exp`、`potion`、`ap`、`ml`、`symphony` 等，不是 Menu 的 `aria:` / `js:` 脚本动作。Prop 的 `cmd` / `msg` 只按本模块规则替换玩家名 token，能力与 `AriaPlayer` 的命令、消息和 PAPI 方法不同。
...
| `sy` / `symphony` | `属性ID:数值` 或 `属性ID:数值%` | 附加 Symphony 属性，持续时间取 `Duration`，如 `critical_damage:8`；末尾 `%` 表示百分比加成 |
