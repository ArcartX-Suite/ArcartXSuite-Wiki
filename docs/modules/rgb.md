...
| 必需 | PlaceholderAPI | 注册 `%axsrgb_*%` 占位符，并解析 `text` 中的嵌套 PAPI | 模块不会加载，占位符输出不可用 |
| 可选 | Chat、Tab、Title 等 | 调用 `%axsrgb_*%` 展示渐变文本 | RGB 本身可用，只是没有对应展示入口 |
...
前缀：`%axsrgb_*%`（**必须安装 PlaceholderAPI**）

| 占位符 | 说明 |
| --- | --- |
| `%axsrgb_<条目ID>%` | 输出对应条目的当前渐变帧文本 |

示例：`%axsrgb_welcome%` → 渲染 `welcome` 条目 `text` 的渐变动画当前帧

### 典型使用场景

| 场景 | 写法 |
| --- | --- |
| 聊天前缀 | Chat 模块 `prefix: "%axsrgb_vip_prefix%"` |
| Tab 列表名称 | Tab 定义 `pack.name: "%axsrgb_server_name%"` |
| 称号显示名 | Title 定义 `display-name: "%axsrgb_title_hero%"` |
