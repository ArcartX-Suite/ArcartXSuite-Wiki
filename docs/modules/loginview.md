...
- **QQ 绑定面板**：未绑定 QQ 的 LittleSkin/正版玩家进服后，通过 UI 面板输入绑定验证码完成 QQ 关联后才允许进入服务器
...
- **已绑定 QQ 的 LittleSkin/正版玩家**进服 → 看到绿色提示「身份已验证」+ 「进入服务器」按钮 → 点击即可进服
- **未绑定 QQ 的 LittleSkin/正版玩家**进服 → 显示绑定提示 UI → 在 QQ 群发送 `#绑定 <玩家名>` 获取验证码 → 在 UI 中输入验证码 → 绑定成功后点击「进入服务器」
...
| 玩家状态 | 进服后 UI | 操作流程 |
|---|---|---|
| 已绑定 QQ + 认证账号 | 显示「进入服务器」按钮 | 点击直接进服 |
| 未绑定 QQ + 认证账号 | 显示绑定提示 + 验证码输入框 | ① QQ 群 `#绑定 <玩家名>` → ② UI 输入 6 位验证码 → ③ 验证通过后显示「进入服务器」按钮 |
...
qq-binding:
  enabled: true
  verify-on-login: true          # 未绑定玩家是否必须在 UI 中验证
  unbound-title: "请先绑定QQ"
  unbound-subtitle: "在QQ群发送 #绑定 {name} 获取验证码"
  enter-without-bind: false      # 是否允许未绑定玩家直接进入（推荐 false）
```
