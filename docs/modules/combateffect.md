...
```yaml
# plugins/ArcartX-Suite/modules.yml
modules:
  combateffect:
    enabled: true
```
...
模块内置 3 个 ArcartX UI 文件，首次启动自动导出到 `plugins/ArcartX-Suite/ui/`。
...
1. 启用模块，确认 `combat_kill_effect.yml` 已导出到 `plugins/ArcartX-Suite/ui/`
