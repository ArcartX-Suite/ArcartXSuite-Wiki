...
- **跨服排行** — 基于 Redis 的跨服 Boss 伤害/击杀/参与排行。
...
| 可选 | Redis (Jedis) | 跨服排行榜数据同步 | 跨服排行不可用，仅限本服排行 |
...
首次启用后，模块会在 `plugins/ArcartXSuite/data/entitytracker/` 下生成：
...
基于 Redis 的跨服排行系统，多个服务器的 Boss 战斗数据实时同步汇总。
...
| 类型 | 说明 |
| --- | --- |
| `best_damage` | 全 Boss 最高单次伤害 |
| `boss_damage` | 针对特定 Boss 的最高伤害排行 |
| `kills` | 参与击杀 Boss 的次数（作为击杀者） |
| `participate` | 参与 Boss 战斗的次数（含辅助） |
| `server` | 按服务器汇总的伤害/击杀贡献排行 |
...
| `player_boss_best_damage` | 玩家 Boss 最佳伤害（排行依据） | player_uuid, boss_id, best_damage（UNIQUE 约束） |
| `cross_server_boss_rankings` | 跨服排行缓存 | ranking_type, boss_id, ranking_data(JSON), expire_time |
| `ranking_reward_configs` | 排行奖励配置 | reward_type, ranking_type, rank_start, rank_end, 奖励内容 |
...
5. **启动服务** — `BossTrackerService`（监听 MythicMob 生成/死亡/受伤事件）和 `EntityTargetHudService`
