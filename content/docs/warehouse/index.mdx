...
Warehouse 是 ArcartXSuite 的仓库与银行模块，通过 ArcartX UI 提供个人仓库、共享仓库、多货币银行、定期存款、二级密码等完整功能。所有业务操作均在 AXUI 界面内完成，玩家通过 `/warehouse`（别名 `/wh`）打开主界面。
...
| 自动存入 | 拾取物品时自动存入仓库，支持 MythicMobs 掉落物过滤，玩家可在 UI 中开关个人设置 |
...
| PlaceholderAPI | 外部依赖 | 提供 PlaceholderAPI 占位符（`%axswarehouse_*%`） |
