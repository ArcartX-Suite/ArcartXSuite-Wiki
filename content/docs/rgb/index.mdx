...
RGB 是 ArcartXSuite 的渐变文本渲染模块，提供 **逐字渐变** 与 **动态扫光** 两种文本效果，通过 PlaceholderAPI 对外暴露。模块将 `entries` 目录下定义的渐变色条目渲染为带 ArcartX 颜色码的彩色文本，可直接用于聊天、TAB 列表、称号、UI 文本等任何支持 PlaceholderAPI 的场景。
...
| 条目热重载 | 通过 `/axs rgb reload` 可热重载条目配置，无需重启服务器 |
...
| 模块依赖 | 无 | - | RGB 不依赖其他 ArcartXSuite 模块即可运行 |
...
2. 启动服务器，模块自动生成 `ArcartXRGB.yml` 和 `entries/default.yml`
3. 编辑 `entries/default.yml` 添加自定义渐变色条目
4. 执行 `/axs rgb reload` 热重载
