...
模块内置三套 UI 模板，启用时自动复制到 `plugins/ArcartXSuite/ui/` 目录：
...
1. 将 `ArcartXSuite-Tab-*.jar` 放入 `plugins/ArcartXSuite/modules/` 目录
2. 在宿主 `config.yml` 中启用模块：`modules.tab.enabled: true`
3. 重启服务端，模块自动导出 `ArcartXTab.yml`、`tabs/` 目录和三套 UI 模板
