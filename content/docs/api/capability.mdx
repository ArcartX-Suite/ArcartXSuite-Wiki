...
地图导航能力（**公开 capability**，`@PublicCapability` + `@Stable`）。由 Map 模块实现。地图是展示层与事件源：来源推送外部目标、接收导航点击回调；导航的实际执行由来源负责，或经 `useMapNavigation`/`navigateExternalTarget` 交给地图内置导航栈代执行。按场景选型的完整对接方案见 [Map - 第三方插件对接方案](/docs/map/integration#第三方插件对接方案)。
...
public interface MapNavigable {
    ...
    boolean navigateExternalTarget(Player player, String targetId);
    ...
    void setNavigationDelegate(Plugin owner, MapNavigationDelegate delegate);
    void clearNavigationDelegate(Plugin owner);
...
| `navigateExternalTarget` | 由地图内置导航栈开始导航指定外部目标（"地图代导航"命令式入口，不过全局委托）；已在导航返回 `true` |
...
public interface MapNavigationDelegate {
    // 玩家请求开始导航；true=已接管（地图记录委托态），false=交还原处理方
    boolean onNavigateRequest(Player player, MapNavigationRequest request);
    // 委托态被终止（玩家取消/互斥清除/来源接管/退出关闭），实现需幂等
    default void onCancelRequest(Player player, MapNavigationRequest request) {}
}
```

`MapNavigationRequest` 为 record：`targetType`（`"anchor"`/`"waypoint"`/`"external"`）/ `targetId` / `worldId` / `title` / `x` / `y` / `z` / `source`（地图自有目标固定 `"map"`，外部目标为其注册 source）。标记点（pin）无 Y 坐标、不可导航，不经此接口。

> 委托契约：导航执行（信标/路径/追踪）完全由委托方负责，地图不建客户端路标；委托接管后地图记录一条委托导航态，面板"导航"按钮切换为"取消"；委托接管外部目标时应另调 `setExternalNavigated` 驱动该目标行按钮切换。
