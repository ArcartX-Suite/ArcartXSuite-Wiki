...
模块主配置文件位于 `plugins/ArcartXSuite/data/entitytracker/ArcartXEntityTracker.yml`，重载命令为 `/axs entitytracker reload`。
...
每个 Boss 的配置文件位于 `plugins/ArcartXSuite/data/entitytracker/bosses/` 目录下，文件名（去掉 `.yml`）即为 MythicMob ID（大小写不敏感）。模块首次启动时会自动导出 `ExampleBoss.yml` 示例文件。
