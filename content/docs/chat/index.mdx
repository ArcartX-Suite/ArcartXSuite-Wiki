...
Chat 是 ArcartXSuite 的全频道聊天系统，提供 **频道聊天**、**私聊回复**、**禁言管理**、**社交监听**、**提及通知**、**物品展示** 六合一聊天体验。模块通过 ArcartX 聊天卡片渲染提及、私聊、系统提示与物品预览，支持多频道定义、权限控制、跨服转发、敏感词过滤等高级功能。
...
| 外部依赖 | PlaceholderAPI | 否 | 注册 `%axschat_xxx%` 占位符，频道格式与卡片文本可解析 PAPI |
...
1. 模块启动时读取 `ArcartXChat.yml` 的 `channels-directory` 字段（默认 `chat/channels`）
