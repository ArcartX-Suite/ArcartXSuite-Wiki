---
title: 统一存储层
description: Suite StorageManager、HikariCP 连接池复用、MySQL/SQLite 双模式与自建模式
---

# 统一存储层

Suite 提供 `StorageManager` 统一存储层（自 1.4.0 起），基于 HikariCP 连接池，支持 MySQL 共享池和 SQLite 按模块分配两种模式，以及模块自建模式覆盖。
