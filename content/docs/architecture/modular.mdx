---
title: 模块化架构
description: Suite 模块加载流程、ClassLoader 隔离、生命周期管理、拓扑排序与 Capability 跨模块通信
---

# 模块化架构

Suite 已从单体插件重构为 **宿主 + 模块 Jar** 架构。用户可按需放入模块 Jar，实现功能的自由组合。宿主会自动检测外部模块并跳过内置加载，由 `ModuleRegistry` 统一管理生命周期。
...
`ModuleRegistry.loadAll()` 扫描 `plugins/ArcartX-Suite/modules/` 目录下的 `.jar` 文件，通过 `ModuleDescriptorParser` 从 jar 内 `module.yml` 读取元数据：
