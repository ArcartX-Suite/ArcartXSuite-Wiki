---
title: 多 UI 发包
description: UI Packet 数据结构、ui-id 列表配置与多 UI 同时发包
---

# 多 UI 发包

ArcartX-Suite 大多数模块的 `ui-id` 字段都支持字符串和列表两种格式。当配置为列表时，服务端会把同一个 payload 同时发送给列表中的每一个 UI，适用于一份数据同时驱动多套 UI 展现的场景。

## 适用场景

- 同一个功能需要在全国菜单和小地图 HUD 上同时展示数据
- 为某个模块制作了多套风格的 UI 文件，希望一次性推送数据到所有 UI
...
| EntityTracker | `config.yml` | `boss.settings.ui-id`、`attack-target.settings.ui-id` |
| Announcer | `config.yml` | `settings.ui-id`、`subtitle.settings.ui-id` |
| Conversation | `config.yml` | `client.dialog-ui-id`、`client.selector-ui-id` |
| QuestGPS | `config.yml` | `client.menu-ui-id`、`client.guide-ui-id` |
| OnlineRewards | `config.yml` | `ui.menu-ui-id` |
| Subtitle 字幕组 | `subtitle/groups/*.yml` | 每个字幕组的 `ui-id` |
